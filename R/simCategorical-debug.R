...
generate.values <- function(
	dataSample, dataPop, method, cur.var, excludeLevels, hasNewLevels, newLevels, w, 
	formula.cmd, eps, limit, censor, levelsResponse) {
...
simCategorical2 <- function(dataS, dataP, method, parallel=TRUE) {
  ...
  # check arguments to account for structural zeros
	...
	for(i in additional ) {
		...
		if ( method == "rpart" ) {
			formula.cmd <- paste(i, "~", paste(predNames, collapse = " + "))
			formula.cmd <- paste("suppressWarnings(rpart(", formula.cmd, 
				 ", weights=", w, ", data=dataSample, method='class'))", sep="")						
		}
		if ( method == "naivebayes" ) {
			formula.cmd <- paste(i, "~", paste(predNames, collapse = " + "))
			formula.cmd <- paste("suppressWarnings(NaiveBayes(", formula.cmd, 
				", data=dataSample, usekernel=TRUE))", sep="")						
		}	
		if ( method == "liblinear" ) {
