simContinuous <- function(synthPopObj, additional = "netIncome",
  ...
  if ( method == "multinom" ) {
    ...
    if ( haveBreaks ) {
      ...
      breaks <- getBreaks(additionalS, dataS[, w], zeros, lower, upper, equidist, probs)
    }
  } else {
    useLm <- TRUE
    if ( log ) {
      if ( is.null(const) ) {
        ...
        if ( haveNeg ) {
          ...
          if ( haveBreaks ) {
            ...
            breaks <- getBreaks(additionalS[neg], dataS[neg, w], zeros=TRUE, lower, upper)
          }
  ...
  if ( useMultinom ) {
    ...
    valuesCat <- lapply(levels(dataS[, strata]), function(s) {
      # sample data
    ...
    valuesCat <- factor(unsplit(valuesCat, dataP[, strata, drop=FALSE]), levels=levels(response))
    ...
    valuesCat <- lapply(levels(dataS[, strata]), function(s) {
      ...
      if ( excludeLevels ) {
        ...
        for ( j in predNames[hasNewLevels] ) {
          ...
          grid[, j] <- factor(as.character(grid[, j]), levels=levels(dataS[, j]))
        }
      }
  ...
  coef <- coef(mod)
  # simulate values
  valuesTmp <- lapply(levels(dataS[, strata]), function(s, coef) {
    # sample data
    ...
    #weights <- dataSample[, w]
    # command needs to be constructed as string
    command <- paste("lm(", fstring,", weights=", w, ", data=dataSample)", sep="")
