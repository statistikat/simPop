...
simEUSILC <- function(dataS, hid = "db030", wh = "db090",
    ...
    samp <- specifyInput(data=dataS, hhid=hid, weight=wh, hhsize=hsize, strata=strata, pid=pid)
    dataP <- simStructure(dataS=samp, basicHHvars=structure)
    ...
    if(useMultinom) {
        ...
        dataP <- simContinuous(dataP, additional=income, zeros=zeros, breaks=breaks,
            gpd=gpd, threshold=threshold, est=est, keep=TRUE, maxit=maxit,
            MaxNWts=MaxNWts)
    } else {
        # two-step model
        dataP <- simContinuous(dataP, additional=income, method="lm", zeros=zeros,
