...
simAnnealingDT <- function(data0, totals0, split.level, params){
  
  N <- V1 <- sim_ID <- weight_choose <- weight_choose_new <- NULL
  FreqType <- factor_med_hh <- Freq <- FreqPopPers <- FreqPopHH <- FreqPers <-
    Diff <- RowIndex <- MARGININDEX <- GROUP <- . <- GROUP_pop <- Diff_new <- 
    firstPersonInHousehold <- NULL
  ######################################
  ...
  choose.temp <- params[["choose.temp"]]
  choose.temp.factor <- params[["choose.temp.factor"]]
  scale.redraw <- params[["scale.redraw"]]
  split <- params[["split"]]
  observe.times <- params[["observe.times"]]
  observe.break <- params[["observe.break"]]
  n.forceCooldown <- params[["n.forceCooldown"]]

  # parameters used for c++ code
  # set index for original order
  data0[, firstPersonInHousehold:=!duplicated(hhid), env = list(hhid = hhid)]
  data0[,sim_ID:=.I]

  # initialize other parameter
  size_all <- sizefactor+1
  max_n <- size_all* nd
  med_hh <- data0[firstPersonInHousehold == TRUE, median(as.numeric(as.character(hhsize))), env = list(hhsize = hhsize)]

  cooldown <- 0
  setkeyv(data0,hhid)
  id <- data0[[hhid]]
  size <- as.numeric(as.character(data0[[hhsize]]))
  ...
  ######################################
  # initialize weights
  data0[, weight_choose := split == split.level, env = list(split = split)]
